...
public class IWLearnedSourceUsageImpl extends PMLObjectImpl implements IWLearnedSourceUsage, Serializable {
  ...
  public IWLearnedSourceUsageImpl() {
  }  
  public IWLearnedSourceUsageImpl(String ontologyURI, String ontologyClassName) {
  	super(ontologyURI, ontologyClassName);
  }
  
  public IWLearnedSourceUsageImpl(String resourceTypeURI) {
  	super(resourceTypeURI);
  }
  
  public IWLearnedSourceUsageImpl(DataObject desc) {
  	super(desc);	
  }
  
	public String getHasDataCollectionStartDateTime() {
		return getDataPropertyValue (PMLP.hasDataCollectionStartDateTime_lname);
	}
	public void setHasDataCollectionStartDateTime(String newDateTime){
		setDataPropertyValue (PMLP.hasDataCollectionStartDateTime_lname, newDateTime);
	}
	public String getHasDataCollectionEndDateTime(){
		return getDataPropertyValue (PMLP.hasDataCollectionEndDateTime_lname);
	}
	public void setHasDataCollectionEndDateTime(String newDateTime){
		setDataPropertyValue (PMLP.hasDataCollectionEndDateTime_lname, newDateTime);
	}
  ...
  public String getSourceInfoStringFormat() {
  	...
  	if (source != null && source.getIdentifier() != null) {
  		result = source.getIdentifier().getURIString() + delimChar + 
  							getQueryString() +
...
//System.out.println("IWSourceUsage.getSourceUsageStringForDocumentGragmentSource: source rowcal type ");
  								...
  								System.out.println("IWSourceUsage.getSourceUsageStringForDocumentGragmentSource: source unrecongnized fragment type "+srcDocType+". ");
  							}
  						} catch (Exception e) {
  ...
  public boolean isSourceDocumentFragment() {
  	...
  	IWSource source = getHasSource();
  	if (source.getOntologyClassName().startsWith("DocumentFragment")) {
  		result = true;
  	}
  	return result;
...
//System.out.println("isV1SourceUsage: "+result);
  ...
  private void setNamePropertyName() {
  	if (properties != null) {
  		...
  		System.out.println("IWLearnedSourceUsage: properties map null");
  	}
  }
...
} /* END OF IWLearnedSourceUsageImpl */
